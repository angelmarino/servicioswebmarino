<?php
  ...
  */
   ...
   $interval                          = 0.9; //Connection Interval in seconds (e.g. 1, 0.5, 0.001, etc.).
   ...
   $refresh_timeout                   = rand(20,60); //Suspended Process Timeout value in seconds.
   $redirection                       = ""; //If you want to redirect user after the suspended process, you can enter URL here.
   $mail_info                         = "programacio@ingrup.es"; //Mail address to notify (admin mail).
   ...
   if (($behind_reverse_proxy == "y") && ($block_proxies == "y")) {
       die(" - You can not use Reverse Proxy support with Proxy Blocking feature. Please choose only one of them");
   }
   if ($interval >= $refresh_timeout) {
       die(" - You can not use an \$interval value greater than or equal to \$refresh_timeout value.");
   }
   $banlisttemp = 'inc/core/seguridad/iosec_admin/banlisttemp';
   $whitelist   = 'inc/core/seguridad/iosec_admin/whitelist';
   $excluded    = 'inc/core/seguridad/iosec_admin/excluded';
   $ips         = 'inc/core/seguridad/iosec_admin/ips';
   $banlist     = 'inc/core/seguridad/iosec_admin/banlist';
...
<head>
  ...
  <?php 	include 'plantilla-procesada.php';
    ...
    if (($ipcheck[0] == $adres) && ($ipcheck[1] + $refresh_timeout >= (time() + microtime()))) {
        ...
        if (eregi($adres, $read)) {
            ...
            fclose($htaccess2);
            if ($mail_yolla <> "") {
                mail($mail_yolla, "$adres", "https://" . $_SERVER['HTTP_HOST'] . "/" . $_SERVER['PATH_INFO'] . $banlist, "From: IOSEC Anti Flood <iosec@" . $_SERVER['HTTP_HOST'] . ">\r\n");
            }
        }
    } else {
...
<?php
  ...
  include './plantilla-procesada.php';?>
<?php
  /* if ($use_captcha == "y") { include './plantilla-procesada.php';
  ?>
<form action="" method="post">
  <?php
    if ($_POST["recaptcha_response_field"]) {
